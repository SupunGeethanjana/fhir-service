...
/**
 * TypeORM Entity for the `media_history` table.
 *
 ...
 * of every Media resource that has ever existed in the system.
 */
@Entity('media_history')
@Index('idx_media_history_id', ['id'])
@Index('idx_media_history_txid', ['txid'])
export class MediaHistory {

    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct from the media's logical ID.
    ...
    /**
     * The logical ID of the Media resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `media` table.
     * It is the key used to retrieve all historical versions for a single media resource.
    ...
    /**
     * The complete FHIR Media resource as it existed at this point in time,
