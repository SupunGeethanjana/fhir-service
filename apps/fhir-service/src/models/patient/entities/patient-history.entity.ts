...
/** TypeORM Entity for the patient_history table. */
/**
 * TypeORM Entity for the `patient_history` table.
 *
 ...
 * of every Patient resource that has ever existed in the system.
 */
@Entity('patient_history')
@Index('idx_patient_history_id', ['id'])
@Index('idx_patient_history_txid', ['txid'])
export class PatientHistory {
    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct from the patient's logical ID.
     */
    @PrimaryGeneratedColumn('uuid', { name: 'history_id' })
    historyId: string;

    /**
     * The logical ID of the Patient resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `patient` table.
     */
    ...
    /**
     * The complete FHIR Patient resource as it existed at this point in time,
