...
/**
 * TypeORM Entity for the `location_history` table.
 *
 ...
 * of every Location resource that has ever existed in the system.
 */
@Entity('location_history')
@Index('idx_location_history_id', ['id'])
@Index('idx_location_history_txid', ['txid'])
export class LocationHistory {
    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct from the location's logical ID.
     */
    @PrimaryGeneratedColumn('uuid', { name: 'history_id' })
    historyId: string;

    /**
     * The logical ID of the Location resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `location` table.
     */
    ...
    /**
     * The complete FHIR Location resource as it existed at this point in time,
