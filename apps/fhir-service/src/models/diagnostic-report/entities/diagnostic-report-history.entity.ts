...
@Entity('diagnostic_report_history')
@Index('idx_diagnostic_report_history_id', ['id'])
@Index('idx_diagnostic_report_history_txid', ['txid'])
export class DiagnosticReportHistory {

  /**
   * The primary key for the history row itself.
   * This is a unique UUID for this specific historical entry and is distinct from the diagnostic report's logical ID.
  ...
  /**
   * The logical ID of the DiagnosticReport resource this history entry belongs to.
   * This is the value that is shared with the `id` column in the `diagnostic_report` table.
   * It is the key used to retrieve all historical versions for a single diagnostic report.
  ...
  /**
   * The complete FHIR DiagnosticReport resource as it existed at this point in time,
