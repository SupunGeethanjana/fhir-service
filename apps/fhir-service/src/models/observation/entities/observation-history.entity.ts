...
/**
 * TypeORM Entity for the `observation_history` table.
 * 
 ...
 * of every Observation resource that has ever existed in the system.
 */
@Entity('observation_history')
@Index('idx_observation_history_id', ['id'])
@Index('idx_observation_history_txid', ['txid'])
export class ObservationHistory {

  /**
   * The primary key for the history row itself.
   * This is a unique UUID for this specific historical entry and is distinct from the observation's logical ID.
  ...
  /**
   * The logical ID of the Observation resource this history entry belongs to.
   * This is the value that is shared with the `id` column in the `observation` table.
   * It is the key used to retrieve all historical versions for a single observation.
