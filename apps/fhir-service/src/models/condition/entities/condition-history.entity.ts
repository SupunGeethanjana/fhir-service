...
/** TypeORM Entity for the condition_history table. */
/**
 * TypeORM Entity for the `condition_history` table.
 *
 ...
 * of every Condition resource that has ever existed in the system.
 */
@Entity('condition_history')
@Index('idx_condition_history_id', ['id'])
@Index('idx_condition_history_txid', ['txid'])
export class ConditionHistory {
    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct from the condition's logical ID.
     */
    @PrimaryGeneratedColumn('uuid', { name: 'history_id' })
    historyId: string;

    /**
     * The logical ID of the Condition resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `condition` table.
     */
    ...
    /**
     * The complete FHIR Condition resource as it existed at this point in time,
