...
/**
 * TypeORM Entity for the `medication_request_history` table.
 * 
 ...
 * of every MedicationRequest resource that has ever existed in the system.
 * 
 * Unlike the main `medication_request` table, this history table:
 * - Never updates existing rows (append-only)
 * - Preserves every version of every MedicationRequest resource
 * - Enables FHIR version history queries (_history endpoints)
 * - Provides complete audit trail capabilities
 */
@Entity('medication_request_history')
@Index('idx_medication_request_history_id', ['id'])
@Index('idx_medication_request_history_txid', ['txid'])
export class MedicationRequestHistory {
    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct 
     * from the medication request's logical ID.
     * 
    ...
    /**
     * The logical ID of the MedicationRequest resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `medication_request` table.
     * It is the key used to retrieve all historical versions for a single medication request.
     * 
     ...
     * Each update to the MedicationRequest increments this version number.
     */
     ...
     * This corresponds to when the MedicationRequest was created or updated.
     */
    ...
    /**
     * The complete FHIR MedicationRequest resource as it existed at this point in time,
     * stored as a JSONB object. This contains all the prescription data including:
     * - Patient and prescriber references
     * - Medication information
     * - Dosage instructions
     * - Status and intent
     * - All other FHIR MedicationRequest fields
     */
