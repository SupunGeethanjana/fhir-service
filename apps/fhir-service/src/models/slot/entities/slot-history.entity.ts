...
/**
 * TypeORM Entity for the `slot_history` table.
 *
 ...
 * of every Slot resource that has ever existed in the system.
 */
@Entity('slot_history')
@Index('idx_slot_history_id', ['id'])
@Index('idx_slot_history_txid', ['txid'])
export class SlotHistory {
    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct from the slot's logical ID.
     */
    @PrimaryGeneratedColumn('uuid', { name: 'history_id' })
    historyId: string;

    /**
     * The logical ID of the Slot resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `slot` table.
     */
    ...
    /**
     * The complete FHIR Slot resource as it existed at this point in time,
