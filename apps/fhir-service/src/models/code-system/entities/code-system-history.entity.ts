...
/**
 * TypeORM Entity for the `code_system_history` table.
 * 
 ...
 * of every CodeSystem resource that has ever existed in the system.
 * 
 * Unlike the main `code_system` table, this history table:
 * - Never updates existing rows (append-only)
 * - Preserves every version of every CodeSystem resource
 * - Enables FHIR version history queries (_history endpoints)
 * - Provides complete audit trail capabilities
 */
@Entity('code_system_history')
@Index('idx_code_system_history_id', ['id'])
@Index('idx_code_system_history_txid', ['txid'])
export class CodeSystemHistory {
    /**
    ...
    /**
     * The FHIR logical ID of the CodeSystem resource (e.g., the "12345" in "CodeSystem/12345").
    ...
    /**
     * The complete FHIR CodeSystem resource as it existed at this point in time,
     * stored as a JSONB object. This contains all the clinical data including:
     * - Code system definition and metadata
     * - Concept definitions with codes and displays
     * - Properties and hierarchies
     * - Publication status and versioning information
     * - All other FHIR CodeSystem fields
     */
