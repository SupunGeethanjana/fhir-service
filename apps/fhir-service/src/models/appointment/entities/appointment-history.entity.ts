...
/**
 * TypeORM Entity for the `appointment_history` table.
 * 
 ...
 * of every Appointment resource that has ever existed in the system.
 * 
 * Unlike the main `appointment` table, this history table:
 * - Never updates existing rows (append-only)
 * - Preserves every version of every Appointment resource
...
@Index('idx_appointment_history_id', ['id'])
@Index('idx_appointment_history_txid', ['txid'])
export class AppointmentHistory {
     ...
     * from the appointment's logical ID.
     * 
    ...
    /**
     * The logical ID of the Appointment resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `appointment` table.
     * It is the key used to retrieve all historical versions for a single appointment.
     * 
     ...
     * Each update to the Appointment increments this version number.
     */
     ...
     * This corresponds to when the Appointment was created or updated.
     */
    ...
    /**
     * The complete FHIR Appointment resource as it existed at this point in time,
     * stored as a JSONB object. This contains all the scheduling data including:
