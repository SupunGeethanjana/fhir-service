...
/**
 * TypeORM Entity for the `organization_history` table.
 *
 ...
 * of every Organization resource that has ever existed in the system.
 */
@Entity('organization_history')
@Index('idx_organization_history_id', ['id'])
@Index('idx_organization_history_txid', ['txid'])
export class OrganizationHistory {
    /**
     * The primary key for the history row itself.
     * This is a unique UUID for this specific historical entry and is distinct from the organization's logical ID.
     */
    @PrimaryGeneratedColumn('uuid', { name: 'history_id' })
    historyId: string;

    /**
     * The logical ID of the Organization resource this history entry belongs to.
     * This is the value that is shared with the `id` column in the `organization` table.
     */
    ...
    /**
     * The complete FHIR Organization resource as it existed at this point in time,
