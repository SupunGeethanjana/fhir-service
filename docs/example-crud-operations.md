# FHIR Transaction Service - Complete CRUD Operations (POST, PUT, PATCH, DELETE)

## Overview

The FHIR Transaction Service now supports complete CRUD operations with advanced features:
...
- **DELETE**: Removes existing resources from the system
...
- Removes existing resource with specified ID from the system
- Requires resource to exist before deletion (validates existence first)
- Returns HTTP 204 (No Content) for successful deletions
- Supports reference validation within bundle context

### Safety Checks
- **Existence Validation**: Verifies resource exists before attempting deletion
- **Transactional Safety**: All deletions are part of atomic transaction
- **Error Handling**: Clear error messages for missing resources
- **Audit Support**: Deletion operations are logged for audit trails
...
|---------|------|-----|-------|--------|
| **ID Handling** | System generates ID | Must specify ID in URL and resource | Must specify ID in URL | Must specify ID in URL |
| **Duplicate Detection** | Smart detection for Patient resources | Not applicable (updates by ID) | Not applicable (updates by ID) | Not applicable |
| **Behavior if Exists** | Creates duplicate or merges (Patients) | Updates the resource | Partially updates the resource | Removes the resource |
| **Behavior if Not Exists** | Creates new resource | Creates with specified ID | Error (resource must exist) | Error (resource must exist) |
| **Data Handling** | Complete resource | Complete resource replacement | Partial update with intelligent merging | No data required |
| **Reference Resolution** | Yes, resolves bundle references | Yes, resolves bundle references | Yes, resolves bundle references | Not applicable |
| **Response Status** | 201 (create) or 200 (existing/updated) | 200 (update) or 201 (create) | 200 (update only) | 204 (no content) |
...
### When to Use DELETE
- Removing obsolete or erroneous resources
- Data cleanup and maintenance operations
- Implementing data retention policies
- Removing test or duplicate data

## Example DELETE Transaction Bundle
...
```json
{
  ...
  "entry": [
    ...
    },
    {
      ...
      "request": {
        "method": "DELETE",
        "url": "Appointment/appointment-cancelled"
      }
    }
  ]
}
...
- Use DELETE for removing obsolete resources
...
- DELETE operations require existing resources and return 204 No Content
